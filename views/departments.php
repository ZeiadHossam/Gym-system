        ...
        <div class="container-fluid">
            ...
            <form role="form" action="../Controller/usertype_controller.php" onsubmit="return validate_depData()" method="get">
				...
				<div class="form-group row">
				  ...
				  <input class="form-check-input" name="receptionCheck" <?php if (isset($_GET['depEditId'])&& $gympages[1]->get_access()==1){ ?> checked <?php }?>  type="checkbox">Reciption
				  <br>
				  <input class="form-check-input" name="notificationCheck" <?php if (isset($_GET['depEditId'])&&$gympages[2]->get_access()==1){ ?> checked <?php }?>  type="checkbox">Notifications
				  <br>
				  <input class="form-check-input" name="membersCheck"   <?php if (isset($_GET['depEditId'])&& $gympages[3]->get_access()==1){ ?> checked <?php }?> type="checkbox">Members
				  <br>
				  <input class="form-check-input" name="employeesCheck" <?php if (isset($_GET['depEditId'])&&$gympages[4]->get_access()==1){ ?> checked <?php }?> type="checkbox">Emloyees

				  </div>
				  <div class="col-sm-2">
				  <input class="form-check-input" name="contractsCheck"  <?php if (isset($_GET['depEditId'])&& $gympages[5]->get_access()==1){ ?> checked <?php }?> type="checkbox">Contracts
				  <br>
				  <input class="form-check-input" name="adminCheck"  <?php if (isset($_GET['depEditId'])&& $gympages[6]->get_access()==1){ ?> checked <?php }?> type="checkbox">Administration
				  <br>
				  <input class="form-check-input"  name="reportsCheck"  <?php if (isset($_GET['depEditId'])&& $gympages[7]->get_access()==1){ ?> checked <?php }?> type="checkbox">Reports
				   ...
				   <div class="btn-group tablebuttons">
                                ...
                                <button type="reset" class="btn btn-danger btn-flat " <?php if (isset($_GET['depEditId'])) { ?>  onclick="cancellingDepartment()"  <?php } ?>>Clear Fields</button>
			...
			<table id="custTable" class="addmembertable table table-bordered table-striped">
                ...
                ?>
					<tr>
						...
						<td>
							 <div class="btn-group tablebuttons">
                                <button type="button" onclick="editingDepartment(<?php echo $department->getId()?>)" class="btn btn-info">Edit
