...
<section class="content">
    ...
    <div class="container-fluid">
        ...
        <form role="form" action="/GYM/package/<?php if (!isset($data['packageId'])){ ?>addPackage <?php }else { ?>editPackage<?php } ?>" onsubmit="return validate_allPackage()" method="get">
            <div class="card-body">
                <div class="form-group row">
                    <label for="Package Name"  class="col-sm-2 col-form-label">Package Name:</label>
                    <div class="col-sm-10">
                        <?php if (isset($data['packageId'])){
                        $packageperiods=$gym->getPackages()[$data['packageId']]->getPeriod();
                        ?>
                        <input type="text" name="packageEditId"
                               value="<?php echo $gym->getPackages()[$data['packageId']]->getId()  ?>" hidden>
                        <input type="Text" onfocusout="validate_packageName()" id="packageName" name="packageName" value="<?php echo $gym->getPackages()[$data['packageId']]->getName() ?>" class="form-control">
                        <?php } else {?>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select name="periodType" id="packageType" class="form-control" onfocusout="validate_packageType()">
                            ...
                            <option <?php if (isset($data['packageId'])&& $gym->getPackages()[$data['packageId']]->getPeriodType()=="Sessions"){ ?> selected <?php }?>>Sessions</option>
                            <option <?php if (isset($data['packageId'])&& $gym->getPackages()[$data['packageId']]->getPeriodType()=="Days"){ ?> selected <?php }?>>Days</option>
                            <option <?php if (isset($data['packageId'])&& $gym->getPackages()[$data['packageId']]->getPeriodType()=="Months"){ ?> selected <?php }?>>Months</option>
                        </select>
                ...
                <div class="btn-group tablebuttons">
                    ...
                    <button type="reset" class="btn btn-danger btn-flat " <?php if (isset($data['packageId'])) { ?>  onclick="cancellingPackage()"  <?php } ?> >Clear Fields</button>
